...
public class ChatBot {
    ...
    */
    ...
    public List<String> sendKeywords(List<Food> foods) {
        ...
        int matchIngCount = 0;
        for (Food f : foods) {
            for (Ingredient in : f.getDetectedAllergens()) {
                matchIngCount++;
            }
        }
        String[] matchedIng = new String[matchIngCount];
        String[] foodNames = new String[foods.size()];
        int j = 0;
        for (int i = 0; i < foods.size(); i++) {
            foodNames[i] = foods.get(i).getFood_name();
            List<Ingredient> matchIngs = foods.get(i).getDetectedAllergens();
            for (Ingredient matchIng : matchIngs) {
                matchedIng[j] = matchIng.getName();
                j++;
            }
        }
        int allIngCount = 0;
        ...
        String[] allIngs = new String[allIngCount];
        int k = 0;
        for (int i = 0; i < foods.size(); i++) {
            List<Ingredient> ingredients = foods.get(i).getIngredient();
            for (Ingredient ingredient : ingredients) {
                allIngs[k] = ingredient.getName();
                k++;
            }
        }
        if (context != null) {
